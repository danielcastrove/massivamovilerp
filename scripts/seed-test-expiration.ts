...
async function main() {
  ...
  if (!product) {
    product = await prisma.product.create({
      data: {
        sku: `SEED_EXP_${Date.now()}`,
        name: "Servicio Premium - Prueba Vencimiento",
        type: "RECURRENT",
  ...
  console.log("");
  console.log("Ve a https://massivamovilerp.vercel.app/api/cron/check-expirations");
  console.log("o ejecuta localmente: GET http://localhost:3000/api/cron/check-expirations");
  console.log("");
